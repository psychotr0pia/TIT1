...
export const actualizarEstadoCamara = async (formData, dispatch) => {
  try {
    ...
    if (postResponse.ok) {
      ...
      dispatch({
        type: "SET_historialEstadoCamara",
        payload: historialEstadosData,
      });
      dispatch({
        type: "SET_ESTADO_CAMARA_SELECCIONADA",
        payload: registrosData.find(
          (camara) => camara.id === parseInt(formData.idCamara, 10)
        ),
      });
...
export const agregarYSetearRegistros = async (formData, dispatch) => {
  try {
    const fechaChileISO = new Date().toLocaleString("sv-SE", {
    ...
    if (
      newFormData.tipo.length === 0 ||
      newFormData.fecha.length === 0 ||
      newFormData.descripcion.length === 0 ||
      newFormData.id_camara.length === 0
    ) {
